...
if __name__ == '__main__':
    bittorrent_description  = 'Torrent Client CLI'
    ...
    parser.add_argument("-s", "--" + SEEDING_DIR_PATH, help="directory path for the seeding file")
    ...
    if(options[DOWNLOAD_DIR_PATH] is None and options[SEEDING_DIR_PATH] is None):
        print('Torrent Client CLI works with either download or upload arguments, try using --help')
        sys.exit()
    
    if options[MAX_PEERS] and int(options[MAX_PEERS]) > 20:
        print("Torrent Client CLI doesn't support more than 20 peer connection !")
        sys.exit()
    
    if options[RATE_LIMIT] and int(options[RATE_LIMIT]) <= 0:
        print("Torrent Client CLI upload / download rate must always greater than 0 Kbps")
        sys.exit()
